...
class Session3Step2Evaluation:
    ...
    def __init__(self, config: Dict):
        ...
        # Load best model from Session 3 Step 1
        self.load_best_available_model()
        
        print(f"Session 3 Step 2 Evaluation initialized")
        print(f"Model parameters: {sum(p.numel() for p in self.model.parameters()):,}")
        print(f"Device: {self.device}")
    
    def load_best_available_model(self):

        # Find latest Session 3 Step 1 run directory
        session3_step1_base = Path('../checkpoints/training_sessions/session_3/step_1')
        run_dirs = list(session3_step1_base.glob('run_*'))
        
        if run_dirs:
            # Find newest run directory
            newest_run = max(run_dirs, key=lambda p: p.stat().st_mtime)
            print(f"[SEARCH] Found latest Session 3 run: {newest_run.name}")
            
            # Find best models in that run
            best_models = list(newest_run.glob('session3_*_best.pt'))
            
            if best_models:
                latest_model = max(best_models, key=lambda p: p.stat().st_mtime)
                try:
                    print(f"[OK] Loading Session 3 model: {latest_model}")
                    checkpoint = torch.load(latest_model, map_location=self.device)
                    self.model.load_state_dict(checkpoint['model_state_dict'])
                    print(f"[TARGET] Session 3 model loaded for evaluation")
                except Exception as e:
                    print(f"[ERROR] Error loading model: {e}")
        else:
            print("[WARNING]  No Session 3 run directories found, using random initialization")
    
    def load_evaluation_data(self):

        data_path = Path('../checkpoints/training_sessions/session_3/step_1/session3_mixed_temporal_eval_data.pkl')
        
        if not data_path.exists():
            raise FileNotFoundError(f"Could not find Session 3 evaluation data: {data_path}")
        
        print(f"[OK] Loading Session 3 evaluation data from: {data_path}")
        with open(data_path, 'rb') as f:
            data = pickle.load(f)
        
        eval_samples = data['samples']
        std_params = data.get('standardization_params', {
            'temperature': {'mean': 0.0, 'std': 100.91},
            'wind': {'mean': 0.0, 'std': 0.95}
        })
        
        print(f"[DATA] Loaded {len(eval_samples)} Session 3 temporal evaluation samples")
    ...
    def evaluate(self):
        ...
        print("[SEARCH] Running comprehensive Session 3 temporal evaluation...")
        ...
        run_dir = Path(f'../checkpoints/training_sessions/session_3/step_2/run_{timestamp}')
    ...
    def run_comprehensive_evaluation_with_matrices(self, eval_loader, std_params, run_dir):
        ...
        print("[TEST] COMPREHENSIVE TEMPORAL EVALUATION WITH MATRIX SAVING")
        print("=" * 60)
        ...
        print(f"[SAVE] Saved matrices and visualizations for {sample_count} temporal evaluation samples")
    ...
    def save_sample_matrices(self, predictions, target_output, sample_idx, run_dir, sample_count, std_params):
        ...
        error_info = {
            ...
            'evaluation_type': 'temporal_dynamics'
        }
    ...
    def generate_sample_visualization(self, pred_temp, target_temp, pred_wind, target_wind, 
        ...
        fig.suptitle(f'Temporal Sample {sample_idx:03d} - Temp MAE: {temp_mae:.1f}K, Wind MAE: {wind_mae:.2f}m/s')
        ...
        # Save plot
        plt.tight_layout()
        plot_path = run_dir / f'sample_{sample_idx:03d}_temporal_visualization.png'
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        plt.close()
        
        print(f"   [EMOJI] Saved temporal visualization: {plot_path.name}")
...
def main():
    ...
    print("[TARGET] FireAIDSS Session 3 Step 2: Temporal Model Evaluation")
    print("Loading: Best model from Session 3 Step 1 latest run")
    print("Purpose: Comprehensive temporal dynamics evaluation")
    print(f"[DATA] Goal: Assess temporal performance with matrix analysis")
    print(f"[SAVE] Saves as: temporal_evaluation_results + matrices + PNGs")
    
    # Initialize wandb logging
    if WANDB_AVAILABLE:
        wandb.init(
            project="fireaidss-training-sessions-2",
            name="session-3-step-2-temporal-evaluation",
            config=config,
            tags=["session3", "step2", "temporal-evaluation", "matrix-analysis"]
        )
    
    try:
        evaluator = Session3Step2Evaluation(config)
        evaluator.evaluate()
